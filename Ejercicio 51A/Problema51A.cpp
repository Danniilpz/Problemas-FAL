...
void resolver(int m, int n, tmatriz const& satis, int k, tSol& s, std::vector <bool>& asignados, int& satisMejor) {
  for (int i=0;i<m;i++) {
    s.sol[k]=i;
    if (!asignados[i]) { // esValida
        ...
        if (k == n-1) { // es solucion
            ...
            resolver(m,n,satis, k + 1, s, asignados, satisMejor);
        }
...
bool resuelveCaso() {
  // Lectura de los datos de entrada
    ...
    resolver(numJuguetes,numNinos,satis, 0, s, asignados, satisMejor);
    std::cout << satisMejor << '\n';
    return true;
}
int main() {
