...
void escribirArray(std::vector<std::pair<int, int>> v) {
    for (std::pair<int, int> n : v) std::cout << n.second << " ";
    std::cout << "\n";
    for (std::pair<int, int> n : v) std::cout << n.first << " ";
    std::cout << "\n";
}
void escribirArrayBool(std::vector<bool> v) {
...
bool resuelveCaso() {
    ...
    //escribirArray(tiempoPropina);
    //Inicializo una solución
    /*int tiempoAux = 0;
    for (int i = 0; i < paquetes; i++) {
        if (tiempoPropina[i].first <= tiempoMax && (tiempoAux + tiempoPropina[i].first * 2) <= tiempoMax) {
            s.solMejor[i] = true;

            s.propinaMejor += tiempoPropina[i].second;
            tiempoAux += tiempoPropina[i].first * 2;
        }
    }
    s.tAux = tiempoAux;*/
